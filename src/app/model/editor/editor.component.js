"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var email_service_1 = require("../../services/email/email.service");
var EditorComponent = (function () {
    function EditorComponent(service) {
        this.service = service;
        this.ckeditorContent = "";
        this.editUnactivated = true;
    }
    EditorComponent.prototype.onChange = function (event) {
    };
    EditorComponent.prototype.onReady = function (event) {
    };
    EditorComponent.prototype.onFocus = function (event) {
    };
    EditorComponent.prototype.onBlur = function (event) {
    };
    EditorComponent.prototype.getCkeditorContent = function () {
        return this.ckeditorContent;
    };
    EditorComponent.prototype.setCkeditorContent = function (ckeditorContent) {
        this.ckeditorContent = ckeditorContent;
    };
    EditorComponent.prototype.getObjet = function () {
        return this.objet;
    };
    EditorComponent.prototype.setObjet = function (objet) {
        this.objet = objet;
    };
    EditorComponent.prototype.getCorps = function () {
        return this.corps;
    };
    EditorComponent.prototype.setCorps = function (corps) {
        this.corps = corps;
    };
    EditorComponent.prototype.activateEditMode = function () {
        this.editUnactivated = false;
    };
    EditorComponent.prototype.desactivateEditMode = function () {
        this.editUnactivated = true;
    };
    EditorComponent.prototype.sendEmailHTML = function (event) {
        console.log('sendEmailComponent FIRED');
        this.service.sendEmailHTML(this.objet, this.ckeditorContent);
    };
    return EditorComponent;
}());
EditorComponent = __decorate([
    core_1.Component({
        selector: 'email-editor',
        templateUrl: './editor.component.html',
        styleUrls: ['./editor.component.css'],
    }),
    __metadata("design:paramtypes", [email_service_1.EmailService])
], EditorComponent);
exports.EditorComponent = EditorComponent;
//# sourceMappingURL=editor.component.js.map